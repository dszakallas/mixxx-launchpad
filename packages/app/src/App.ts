...
export default class App extends Component {
  ...
  updateLayout(diff: Diff) {
    ...
    addedBlocks.forEach((block) => {
      ...
      if (block.index) {
        this.device.sendColor(
          this.bindings[block.channel][0].control,
          this.device.colors.hi_orange
        );
      } else {
        this.device.sendColor(
          this.bindings[block.channel][0].control,
          this.device.colors.hi_green
        );
      }
  ...
  addToChord(channel: number) {
    ...
    this.device.sendColor(
      this.bindings[channel][0].control,
      this.device.colors.hi_red
    );
  }
  ...
  return current.reduce(
    (diff, block) => {
      ...
      return matched === -1
        ...
        : [
            neg,
            pos.slice(0, matched).concat(pos.slice(matched + 1, pos.length)),
          ];
    },
    [[], next] as Diff
  );
...
const cycle = (
  ...
  const nextIndex = posMod(
    current[matched].index + dir,
    lengths[current[matched].size].length
  );
  ...
  return [
    [current[matched]],
    [Object.assign({}, current[matched], { index: nextIndex })],
  ];
};
